...
const mainMenu = [
  { name: "Home", icon: Home, href: "/Editor/dashboard" },
  { name: "Projects", icon: Film, href: "/Editor/dashboard/projects" },
  { name: "Media Library", icon: FileVideo, href: "/Editor/dashboard/media" },
];

const libraryMenu = [
  { name: "Tasks", icon: PlayCircle, href: "/Editor/dashboard/tasks" },
  { name: "Chat", icon: MessageCircle, href: "/Editor/dashboard/chat" },
  { name: "Schedule", icon: Calendar, href: "/Editor/dashboard/schedule" },
  { name: "Notifications", icon: Bell, href: "/Editor/dashboard/notifications" },
  { name: "Performance", icon: BarChart, href: "/Editor/dashboard/performance" },
  { name: "Profile", icon: UserCircle, href: "/Editor/dashboard/profile" },
];
    ...
    <aside className={`w-64 bg-white border-r border-gray-100 h-full flex flex-col ${!open ? 'hidden' : 'flex'} md:flex`}>
      <div className="p-4 flex items-center justify-center border-b border-gray-100 mb-4">
        <div className="bg-[#fff1ec] p-5 rounded-xl flex items-center">
          ...
          />
          <span className="ml-3 font-bold text-lg text-[#ff4e00]">Editor Panel</span>
        </div>
      </div>
      
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-6">
        <MenuSection items={mainMenu} />
        <MenuSection title="TOOLS" items={libraryMenu} />
      </div>
