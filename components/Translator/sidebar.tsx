...
const mainMenu = [
  { name: "Home", icon: Home, href: "/Translator/dashboard" },
  { name: "Projects", icon: Film, href: "/Translator/dashboard/projects" },
  { name: "Translations", icon: Globe, href: "/Translator/dashboard/translations" },
];

const workMenu = [
  { name: "Pending Tasks", icon: Clock, href: "/Translator/dashboard/pending" },
  { name: "Completed", icon: CheckSquare, href: "/Translator/dashboard/completed" },
  { name: "Glossary", icon: BookOpen, href: "/Translator/dashboard/glossary" },
  { name: "Messages", icon: MessageCircle, href: "/Translator/dashboard/messages" },
  { name: "Schedule", icon: Calendar, href: "/Translator/dashboard/schedule" },
  { name: "Notifications", icon: Bell, href: "/Translator/dashboard/notifications" },
  { name: "Performance", icon: BarChart, href: "/Translator/dashboard/performance" },
  { name: "Profile", icon: UserCircle, href: "/Translator/dashboard/profile" },
];
    ...
    <aside className={`w-64 bg-white border-r border-gray-100 h-full flex flex-col ${!open ? 'hidden' : 'flex'} md:flex`}>
      <div className="p-4 flex items-center justify-center border-b border-gray-100 mb-4">
        <div className="bg-[#fff1ec] p-5 rounded-xl flex items-center">
          ...
          />
          <span className="ml-3 font-bold text-lg text-[#ff4e00]">Translator</span>
        </div>
      </div>
      
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-6">
        <MenuSection items={mainMenu} />
        <MenuSection title="WORKSPACE" items={workMenu} />
      </div>
