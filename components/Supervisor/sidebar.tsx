...
const mainMenu = [
  { name: "Home", icon: Home, href: "/Supervisor/dashboard" },
  { name: "Discovery", icon: Compass, href: "/Supervisor/dashboard/discovery" },
  { name: "Users", icon: Users, href: "/Supervisor/dashboard/users" },
];

const libraryMenu = [
  { name: "Tasks", icon: PlayCircle, href: "/Supervisor/dashboard/tasks" },
  { name: "Chat", icon: MessageCircle, href: "/Supervisor/dashboard/chat" },
  { name: "Schedule", icon: Calendar, href: "/Supervisor/dashboard/schedule" },
  { name: "Notifications", icon: Bell, href: "/Supervisor/dashboard/notifications" },
  { name: "Performance", icon: BarChart, href: "/Supervisor/dashboard/performance" },
  { name: "Commission", icon: DollarSign, href: "/Supervisor/dashboard/commission" },
  { name: "Profile", icon: UserCircle, href: "/Supervisor/dashboard/profile" },
];
    ...
    <aside className={`w-64 bg-white border-r border-gray-100 h-full flex flex-col ${!open ? 'hidden' : 'flex'} md:flex`}>
      <div className="p-4 flex items-center justify-center border-b border-gray-100 mb-4">
        <div className="bg-[#fff1ec] p-5 rounded-xl flex items-center">
          ...
          />
          <span className="ml-3 font-bold text-lg text-[#ff4e00]">Astaan Films</span>
        </div>
      </div>
      
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-6">
        <MenuSection items={mainMenu} />
        <MenuSection title="LIBRARY" items={libraryMenu} />
      </div>
